...
clin_feat = fread("clinical.ICGC-BRCA-EU_BRCA_Davies2017.tsv")
...
mean(auc_dt_LOG_t$AUC_glm)
mean(auc_dt_LOG_t$AUC_svm)
mean(auc_dt_LOG_t$AUC_RF)


auc_dt_LOG_t = auc_dt[Normalize==TRUE]
auc_dt_LOG_f = auc_dt[Normalize==FALSE]
...
auc_dt_LOG_t = auc_dt[Raw_or_proc=="both"]

mean(auc_dt_LOG_t$AUC_glm)
mean(auc_dt_LOG_t$AUC_svm)
mean(auc_dt_LOG_t$AUC_RF)

median(auc_dt_LOG_t$AUC_glm)
median(auc_dt_LOG_t$AUC_svm)
median(auc_dt_LOG_t$AUC_RF)

auc_dt_LOG_t = auc_dt[Raw_or_proc=="raw"]
...
auc_dt_LOG_t = auc_dt[Raw_or_proc=="proc"]
...
auc_dt_LOG_t = auc_dt[Normalize==FALSE & Log_transform==FALSE]
...
auc_dt_LOG_t = auc_dt[Normalize==TRUE & Log_transform==FALSE]
...
auc_dt_LOG_t = auc_dt[Normalize==FALSE & Log_transform==TRUE]
...
auc_dt_LOG_t = auc_dt[Normalize==TRUE & Log_transform==TRUE]
...
auc_melt = melt(auc_auc, id.vars=c("Set_num", "Raw_or_proc", "Normalize", "Log_transform"))
setnames(auc_melt, "variable", "ML_method")
setnames(auc_melt, "value", "AUC")

auc_rank = auc_dt[,.SD,.SDcols=c(auc_cols[1:4], auc_cols[8:10])]
rank_melt = melt(auc_rank, id.vars=c("Set_num", "Raw_or_proc", "Normalize", "Log_transform"))
...
ggplot(data=auc_melt)+
    geom_violin(aes(x=ML_method, y=AUC, color=ML_method, fill=ML_method))+
    theme_bw()+
    ggtitle("AUC by Machine Learning Method & Normalize (T/F)")+
    theme(plot.title = element_text(hjust=.5))+
    facet_grid(.~Normalize)

ggplot(data=auc_melt)+
    geom_violin(aes(x=ML_method, y=AUC, color=ML_method, fill=ML_method))+
    theme_bw()+
    ggtitle("AUC by Machine Learning Method & Log_transform (T/F)")+
    theme(plot.title = element_text(hjust=.5))+
    facet_grid(.~Log_transform)

rank_melt[,Rank:=as.integer(Rank)]

ggplot(data=rank_melt)+
...
case_pos = length(unique(status_dt[isQuiescentGenomeControl==FALSE, Sample_id_1]))
case_neg = length(unique(status_dt[isQuiescentGenomeControl==TRUE, Sample_id_1]))
